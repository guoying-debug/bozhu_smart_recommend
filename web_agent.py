...
from app.core.agent import init_bilibili_agent
...
with st.sidebar:
    ...
    **提示**：尝试输入 *"帮我优化标题：Python入门教程"*
    ...
    if st.button("🧹 清除对话历史"):
        ...
        # 重建 agent，清空其内部 memory，避免旧上下文残留
        st.session_state.agent = init_bilibili_agent()
        st.rerun()

# 聊天主界面
st.title("💬 B站运营 Agent 聊天室")

# 初始化聊天记录和每用户独立 agent（避免多用户共享记忆）
...
if prompt := st.chat_input("请输入你的问题，例如：帮我优化标题：Python从入门到放弃"):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Agent 正在思考并调用工具..."):
            # 经 harness 统一调度：错误降级、耗时统计、工具命中记录
            result = harness.handle(prompt, st.session_state.agent)
            st.markdown(result.output)
            if result.tool_used:
                st.caption(f"🔧 调用工具：{result.tool_used} · 耗时 {result.latency_ms}ms")
